...
export const getCustomerHierarchy = async function getCustomerHierarchy(parentId, database, processedIds = new Set()) {
    try {
        ...
        const [users, customers] = await Promise.all([
            User.find({ created_by: parentId,database:database, status: 'Active' }).lean(),
            Customer.find({ created_by: parentId,database:database, status: 'Active', leadStatusCheck: "false" }).populate({ path: "created_by", model: "user" }).populate({ path: "category", model: "customerGroup" }).populate({ path: "rolename", model: "role" }).populate({ path: "assignTransporter._id", model: "transporter" }).lean()
        ]);
